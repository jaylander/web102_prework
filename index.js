...
function addGamesToPage(games) {
    ...
    for (const game in games) {
        ...
        divElement.innerHTML = `
            <img class='game-img' src="${games[game].img}" alt='${games[game].name}'/>
...
let numOfGames = Object(GAMES_JSON).length;
...
let numOfUnfundedGames = GAMES_JSON.reduce( (num, game) => {
    return num + (game.goal > game.pledged);
}, 0);

// create a string that explains the number of unfunded games using the ternary operator

const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${numOfGames}. Currently, ${(numOfUnfundedGames == 1) ? numOfUnfundedGames + " game": numOfUnfundedGames + " games"} remain unfunded. We need your help to fund these amazing games!`;
